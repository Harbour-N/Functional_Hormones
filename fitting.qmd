...
bs3_lb = -np.pi * 2/3
bs3_ub = np.pi * 1/4
bs_bounds = [bs1_lb, bs1_ub, bs2_lb, bs2_ub, bs3_lb, bs3_ub]
    ...
    if accuracy > nRMSE_threshold:
        ...
        for i in range(100):
            for k in range(n_basis):
                ...
                if k == 0:
                    guess[3*k] = np.random.uniform(bs1_lb,bs1_ub)
              
                elif k == 1:
                    guess[3*k] = np.random.uniform(bs2_lb,bs2_ub)
                elif k == 2:
                    guess[3*k] = np.random.uniform(bs3_lb,bs3_ub)
            ...
            popt, pcov = curve_fit(func, t, time_series, p0=guess, maxfev=1000000000)
            fit = func(t, *popt)
            accuracy = NRMSE(time_series, fit)
