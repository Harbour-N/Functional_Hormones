...
new_df = circle_shift(df)

# Z (mean) transform the data
num_Z = np.zeros(num.shape)
for i in range(len(num[:,0])):
    num_Z[i,:] = (num[i,:] - np.mean(num[i,:]))/np.std(num[i,:])
...
PIDs = new_df['PID'].unique()

# First consider patients which have exactly 72 time points
for PID in PIDs:

    # Get the data for this PID
    data = new_df[new_df['PID'] == PID]
    # Check if the number of time points is 72
    
    # Plot the data
    plt.plot(data['NewTime'], data['Cortisol'], label=PID)
    plt.xlabel('Time')
    plt.ylabel('Cortisol')
    plt.title(f'Cortisol levels for {PID}')
