---
title: Presenation Figures
description: Make figures for catch up presentation with Eder and Clinicians
authors:
...
def plot_single(popt, t, time_series,ax, ss_res = -1):
    ax.plot(t, time_series, 'bo-',linewidth=4)
    ax.plot(t, func(t, *popt) , 'r-',linewidth=4)
    ...
    for i in range(0, len(popt), 3):
        ...
        wid = popt[i+2]
        y = amp*np.exp(wid*np.cos(t-ctr))/(2*np.pi*i0(wid))
        ax.plot(t, y,'--', alpha = 0.8, linewidth=4)
...
# specify the participants to plot to make nice figure
ind_to_plot = [0,1,31,59,13,5,6]
p = 0
...
for i in range(rows):
    for j in range(cols):
      ...
      if i*10 + j >= len(num):
          break
      plot_single(basis_save[ind_to_plot[p],:], t, num[ind_to_plot[p],:], ax[i,j], ss_res_save[ind_to_plot[p]])
      p = p + 1
...
for i in range(rows):
    for j in range(cols):
        if i < rows - 1:  # If not in the last row
            ...
            ax[i, j].set_xticks(np.linspace(-np.pi,np.pi,7), ['12', '16', '20', '00', '04', '08', '12'],fontsize = 18)  # Set x-tick labels

# Add a legend at the center top of the figure
fig.legend(['Data', 'Fit', 'Basis 1', 'Basis 2', 'Basis 3'], loc='upper center', ncol=5, fontsize=20, bbox_to_anchor=(0.5, 0.93))
